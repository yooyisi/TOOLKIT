...
class TextCNN(object):
    ...
    def min_max_scale(self, x):
        return (x - tf.expand_dims(tf.reduce_min(input_tensor=x, axis=1), axis=1)) / tf.expand_dims(
            (tf.reduce_max(input_tensor=x, axis=1) - tf.reduce_min(input_tensor=x, axis=1)), axis=1)

    def standard_scale(self, x):
        mean, variance = tf.nn.moments(x=x, axes=[1])
